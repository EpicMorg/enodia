...
`)
	...
	exportFromFlag, exportFormatFlag, exportOutputFlag = invPath, "json", "-"
	...
	var got []evaluate.Assessment
	if err := json.Unmarshal(stdout.Bytes(), &got); err != nil {
		t.Fatalf("output is not valid JSON: %v\n%s", err, stdout.String())
	}
	if len(got) != 1 || got[0].ID != "x" || got[0].Patch != evaluate.PatchBehind {
		t.Fatalf("got %+v", got)
	}
}

func TestRunExportCmdUnsupportedFormatIsBadArgument(t *testing.T) {
	prevFormat := exportFormatFlag
	exportFormatFlag = "html"
	t.Cleanup(func() { exportFormatFlag = prevFormat })
...
`)
	...
	prevFrom, prevFormat, prevOut := exportFromFlag, exportFormatFlag, exportOutputFlag
	exportFromFlag, exportFormatFlag, exportOutputFlag = invPath, "json", outPath
	t.Cleanup(func() { exportFromFlag, exportFormatFlag, exportOutputFlag = prevFrom, prevFormat, prevOut })
