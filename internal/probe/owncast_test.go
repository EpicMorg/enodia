...
func TestOwncastProbeMeta(t *testing.T) {
	...
	if m.DefaultResolver.Type != "github" || m.DefaultResolver.ID != "owncast/owncast" {
		t.Fatalf("got resolver %+v, want github/owncast/owncast (no endoflife.date calendar)", m.DefaultResolver)
	}
}
